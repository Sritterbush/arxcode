    ...
    def event(self):
        ...
        if roll <= 50:
            ...
            if domain.land == self.land:
                ...
                if b_roll == HOUSE:
                    domain.num_housing += 1
                elif b_roll == FARM:
                    domain.num_farms += 1
                elif b_roll == MILL:
                    domain.num_mills += 1
                elif b_roll == LUMBER:
                    domain.num_lumber_yards += 1
                elif b_roll == MINE:
                    domain.num_mines += 1
                domain.area += 1
                domain.save()
        # fight, and if we win we get the land
            ...
            bandit_units = bandit_army.units.create(type=unit_types.INFANTRY, quantity=100*area)
            ...
            aban_units = aban_army.units.create(type=unit_types.INFANTRY, quantity=100*area)
